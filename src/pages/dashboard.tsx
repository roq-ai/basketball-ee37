...
import { RefereeListPage as ShowCaseList } from 'pages/referees';
...
function DashboardPage() {
  ...
  const gameStat = useSWR<PaginatedInterface<GameInterface>>(
    () => `/games`,
    () => getGames({ limit: 1, offset: 0 }),
  );

  const refereeStat = useSWR<PaginatedInterface<RefereeInterface>>(
    () => `/referees`,
    () => getReferees({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Games',
      result: gameStat?.data?.totalCount,
      isLoading: gameStat.isLoading,
    },
    {
      title: 'Referees',
      result: refereeStat?.data?.totalCount,
      isLoading: refereeStat.isLoading,
    },
  ];
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`referees`} passHref legacyBehavior>
                  <Button
