...
type ColumnType = ColumnDef<GameInterface, unknown>;
...
export function GameListPage(props: GameListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<GameInterface>>(
    () => `/games?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('referee', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          accessorKey: 'referee',
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/referees/view/${record.referee?.id}`}>
              {record.referee?.id}
            </Link>
          ),
        }
    ...
    hasAccess('team', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'team_game_team1_idToteam',
          header: 'Team Game Team 1 Id Toteam',
          accessorKey: 'team_game_team1_idToteam',
          cell: ({ row: { original: record } }: any) => (
            <Link
              as={NextLink}
              onClick={(e) => e.stopPropagation()}
              href={`/teams/view/${record.team_game_team1_idToteam?.id}`}
            >
              {record.team_game_team1_idToteam?.name}
            </Link>
          ),
        }
      : null,
    hasAccess('team', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'team_game_team2_idToteam',
          header: 'Team Game Team 2 Id Toteam',
          accessorKey: 'team_game_team2_idToteam',
          cell: ({ row: { original: record } }: any) => (
            <Link
              as={NextLink}
              onClick={(e) => e.stopPropagation()}
              href={`/teams/view/${record.team_game_team2_idToteam?.id}`}
            >
              {record.team_game_team2_idToteam?.name}
            </Link>
          ),
        }
      : null,
